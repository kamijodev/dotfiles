...
- **マジックナンバー/ストリング**: 意味が不明な数値やハードコードされた文字列が直接使われていないか。定数や列挙型で名前を付けて定義すべき。特に `type Foo = 'a' | 'b'` のようなユニオン型を定義している場合、リテラル文字列を直接使わず `as const` オブジェクト定数（例: `const Foo = { A: 'a', B: 'b' } as const`）を定義して参照すること
